<template>
  <div>
    <Dropdown width="300px" :autoClose="false" :isClose="isClose" @close="toggleClose" maxHeight="490px">
      <template slot="button">
        <button :class="['zpl-filter-button', { isActive }]">
          {{ title }}
          <div v-if="isActive">
            <span class="hidden lg:inline-flex">: {{ finalActiveVal }}</span>
            <Icon name="FilledDelete" class="zpl-filter-delete" @click.native="clearHandler" />
          </div>
        ...
        <div class="p-xs">
          <div class="zpl-filter-header">
            ...
            <div v-if="isActive" @click="clearHandler" class="zpl-filter-delete-btn">
              <div>{{ $t('common.delete') }} {{ $t('common.export.filter') }}</div>
